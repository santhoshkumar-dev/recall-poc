  ...
  return (
    <div>
      ...
      <section className="panel mt-8 p-6">
        <div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-lg font-semibold">Indexing queue</h2><p className="mt-1 text-sm text-black/45">{indexing?.currentFile ? `Processing ${indexing.currentFile}` : indexing?.paused ? "Queue is paused" : "Queue is up to date"}</p></div><Button variant="secondary" size="sm" onClick={() => void queueToggle()}>{indexing?.paused ? <><Play size={15} /> Resume</> : <><Pause size={15} /> Pause</>}</Button></div>
        <Progress value={progress} className="mt-5" />
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-5">{[["Pending", indexing?.pending], ["Processing", indexing?.processing], ["Indexed", indexing?.indexed], ["Skipped", indexing?.skipped], ["Failed", indexing?.failed]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-black/[.035] p-4"><div className="text-2xl font-semibold">{value ?? 0}</div><div className="mt-1 text-xs text-black/45">{label}</div></div>)}</div>
      </section>
